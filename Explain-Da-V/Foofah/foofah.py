...
def a_star_search(
    ...
    while not open_q.empty():
        ...
        for c in my_children:
            ...
            if (
                c not in cost_q or (c in cost_q and c.f_score < cost_q[c])
            ) and c.f_score < float("inf"):
                open_q.put(c)
                cost_q[c] = c.f_score
...
def main(
    ...
    parser.add_argument(
        "--auto_read",
        action="store_true",
        help="Automatically read csv file using csv reader",
    )
    ...
    parser.add_argument(
        "--no_batch", action="store_true", default=False, help="Disable batch"
    )
    ...
    parser.add_argument(
        "--p1off", action="store_true", default=False, help="turn off prune rule 1"
    )
    parser.add_argument(
        "--p2off", action="store_true", default=False, help="turn off prune rule 2"
    )
    parser.add_argument(
        "--p3off", action="store_true", default=False, help="turn off prune rule 3"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="turn off global pruning rules",
    )
    parser.add_argument(
        "--opPruneOff",
        action="store_true",
        default=False,
        help="turn off operator pruning rules",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="turn off 1st wrap operator",
    )
    parser.add_argument(
        "--wrap2off",
        action="store_true",
        default=False,
        help="turn off 2nd wrap operator",
    )
    parser.add_argument(
        "--wrap3off",
        action="store_true",
        default=False,
        help="turn off 3rd wrap operator",
    )
    ...
    for test_file in test_files:
        ...
        if what_to_explain == "columns":
            ...
            # print('here')
            search_space = add_ops()
        final_node, open_nodes, closed_nodes = a_star_search(
            raw_data,
            target,
            search_space,
            debug_level,
            timeout=ext_time_limit,
            batch=if_batch,
            epsilon=epsilon,
            bound=bound,
            algo=search_algo,
            ...
            with open("foofah_log", "w") as o:
                with contextlib.redirect_stdout(o):
                    ...
                    for i, n in enumerate(reversed(path)):
                        ...
                        if len(n.operation) > 1:
                            if n.operation[1]:
                                print(
                                    (
                                        "%2d. %-13s at %d: H-score: %.1f Actual: %d"
                                        % (
                                            i + 1,
                                            n.operation[0]["name"],
                                            n.operation[1],
                    ...
                    if final_node.contents != target:
                        print()
                        print(
                            '%2d. Only "Moves" are needed to create a extact same view as target (TO BE COMPLETED).'
                            % (len(path) + 1)
                        )
                        print()
                    ...
                    print(
                        "Time elapsed:    %.3f s   Nodes visited:  %d   Nodes created: %d"
                        % ((end - start), num_visited, nodes_created)
                    )
                    print(
                        "Naive branching factor: %d   Effective branching factor: %.2f"
                        % (len(add_ops()), branch_factor)
                    )
                    print(
                        "Make child time: %.2f s   Heuristic time: %.2f s"
                        % (
                            sum(final_node.times["children"]),
                            sum(final_node.times["scores"]),
                        )
                    )
                    ...
                    if if_validate:
                        ...
                        if test_table:
                            ...
                            print("-" * 50)
                            print(
                                "Experiment 1: Apply the synthetic program on other data"
                            )
                            print("-" * 30)
                            print("Testing Table")
                            ...
                            print(
                                tabulate(
                                    test_data["TransformedTestTable"], tablefmt="grid"
                                )
                            )
                            print("-" * 30)
                        ...
                        filename = (
                            dirname
                            + "/exp0_results_"
                            + str(test_data["TestName"])
                            + "_"
                            + str(test_data["NumSamples"])
                            + ".json"
                        )
